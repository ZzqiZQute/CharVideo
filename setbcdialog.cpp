...
#include "common.h"
#include "converter.h"
SetBCDialog::SetBCDialog(QWidget *parent, QStringList filePath, int charWidth, int charHeight, int finalWidth, int finalHeight, int style,char* usedChar,int charcount) :
    QDialog(parent),ui(new Ui::SetBCDialog),mCharWidth(charWidth),mCharHeight(charHeight),mFinalWidth(finalWidth),mFinalHeight(finalHeight),mStyle(style),mUsedChar(usedChar),mCharCount(charcount)
{
    ui->setupUi(this);
...
void SetBCDialog::drawPic()
{
    ...
    QImage outImage=Converter::convert(mFilePath,mIndex,mCharWidth,mCharHeight,mFinalWidth,mFinalHeight,mStyle,mBrightness,mContrast,mUsedChar,mCharCount);
    mScene->clear();
